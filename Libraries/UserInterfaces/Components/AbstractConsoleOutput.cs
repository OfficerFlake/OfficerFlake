...
{
	public abstract partial class AbstractConsoleOutput : UserControl
    {
		#region Constructor
		internal AbstractConsoleOutput()
	    {
		    InitializeComponent();
		    if (DesignMode) return;
			MessagesMenu.Subscribe(OnMessagesOptionChanged);
			new Thread(() => RichTextBoxUpdaterThread()).Start();
		}
		#endregion

		#region Variables
		protected ClickableMenu MessagesMenu { get; set; } = new ClickableMenu();

	    protected Boolean ShowDate { get; set; } = true;
	    protected Boolean ShowTime { get; set; } = true;
		protected Boolean ShowType { get; set; } = true;
		protected Boolean ShowMessage { get; set; } = true;
		protected Boolean ShowMessage_Unknown { get; set; } = true;
		protected Boolean ShowMessage_ConsoleUser { get; set; }
	    protected Boolean ShowMessage_ConsoleInformation { get; set; }
	    protected Boolean ShowMessage_DebugSummary { get; set; }
	    protected Boolean ShowMessage_DebugDetail { get; set; }
	    protected Boolean ShowMessage_DebugWarning { get; set; }
	    protected Boolean ShowMessage_DebugError { get; set; }
	    protected Boolean ShowMessage_DebugCrash { get; set; }
		#region Closing
		...
		private int MessageSize =>
			91 -
			(ShowDate ? DateSize+1 : 0) -
			(ShowTime ? TimeSize+1 : 0) -
			(ShowType ? TypeSize+1 : 0);
		private int OverflowSize =>
			91 -
			(ShowDate ? DateSize + 1 : 0) -
			(ShowTime ? TimeSize + 1 : 0) -
			(ShowType ? TypeSize + 1 : 0) -
			(ShowMessage ? MessageSize : 0);
		#endregion
	    #endregion
		#region Control Events
		protected void OnMessagesOptionChanged(object sender, EventArgs e)
	    {
		...
		{
			private AbstractConsoleOutput Parent;
			public int StartIndex;
			public IRichTextMessage Message;

			public RichTextMessageDictionaryItem(AbstractConsoleOutput _Parent, IRichTextMessage _Message)
			{
				Parent = _Parent;
		...
		private int GetIndentSize()
		{
			...
			if (ShowDate) output += (DateSize + 1);
			if (ShowTime) output += (TimeSize + 1);
			if (ShowType) output += (TypeSize + 1);
			return output;
		}
	    ...
	    protected void LinkMessagesMenu(ClickableMenu menuToLink)
	    {
		    MessagesMenu = menuToLink;
		}
		private void ShowMessageControlMenu(object sender, EventArgs e)
		{
			MessagesMenu.Show();
			MessagesMenu.Visible = true;
		}
	}
}
