...
namespace Com.OfficerFlake.Libraries.YSFlight
{
	...
	{
		// TODO : NOT DONE! NEED TO REVISE AGAINST OYS.WORLD! (PRIORITY=5);
		...
		{
			if (!File.Exists(Metadata.YSFlightDirectory +  InputScenery.SceneryPath1Fld)) return false;
			...
			for (int i = 0; i < FLDContents.Length; i++)
			{
				...
				else
				{
					...
					#endregion
					#region FIL
					if (ThisLine.ToUpperInvariant().CleanString().SplitPresevingQuotes()[0] == "FIL")
					{
						if (!ThisLine.Split(' ')[1].ToUpperInvariant().Contains(".FLD")) continue;
						...
						if (AllSceneries.Select(x => x.Identify).Contains(ThisLine.ToUpperInvariant().CleanString().SplitPresevingQuotes()[1]))
						{
							//Console.WriteLine("FIL FOUND: " + ThisLine);
							TargetScenery = AllSceneries.Where(x => x.Identify == ThisLine.ToUpperInvariant().CleanString().SplitPresevingQuotes()[1]).ToArray()[0];
							ExpectPos = i + 1;
						}
						continue;
					}
