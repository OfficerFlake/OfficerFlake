    ...
    public partial class ConsoleOutput : IConsole, IDebug
    {
                ...
                try
                {
                    Dispatcher.Invoke(() =>
                    {
                        ...
                        if (newMessage.Type == MessageType.ConsoleInformation &&
                            !SettingsLibrary.Settings.UserInterface.Console.ShowMessages.Console) return;
                        if (newMessage.Type == MessageType.User &&
                            !SettingsLibrary.Settings.UserInterface.Console.ShowMessages.User) return;

                        if (newMessage.Type == MessageType.DebugSummary &&
                            !SettingsLibrary.Settings.UserInterface.Console.ShowDebug.Summary) return;
                        if (newMessage.Type == MessageType.DebugDetail &&
                            !SettingsLibrary.Settings.UserInterface.Console.ShowDebug.Detail) return;
                        if (newMessage.Type == MessageType.DebugWarning &&
                            !SettingsLibrary.Settings.UserInterface.Console.ShowDebug.Warning) return;
                        if (newMessage.Type == MessageType.DebugError &&
                            !SettingsLibrary.Settings.UserInterface.Console.ShowDebug.Error) return;
                        if (newMessage.Type == MessageType.DebugCrash &&
                            !SettingsLibrary.Settings.UserInterface.Console.ShowDebug.Crash) return;
        ...
        public void ClearAllMessages()
        {
            try
            {
                Dispatcher.Invoke(() =>
                {
                _messageList.RemoveAll(x =>
                    x is IConsoleUserMessage | x is IConsoleInformationMessage | x is IDebugSummaryMessage | x is IDebugDetailMessage |
                    x is IDebugWarningMessage | x is IDebugErrorMessage | x is IDebugCrashMessage);
                RefreshGrid();
                });
            }
            catch
        ...
        public void ClearDebugMessages()
        {
            try
            {
                ...
                {
                _messageList.RemoveAll(x =>
                x is IDebugSummaryMessage | x is IDebugDetailMessage | x is IDebugWarningMessage | x is IDebugErrorMessage |
                x is IDebugCrashMessage);
                RefreshGrid();
                });
            }
            catch
